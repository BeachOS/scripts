...
if [[ -e "${top}/build_kernel.sh" ]]; then
  readonly excluded_repos='BeachOS/kernel_manifest'
else
  readonly excluded_repos='BeachOS/platform_manifest'
fi
...
handle_repos() {
  ...
  local repos=`repo list | grep BeachOS | grep -Ev "${excluded_repos}" | cut -d : -f 1 | tr -d ' '`
